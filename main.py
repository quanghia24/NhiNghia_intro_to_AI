...
cell_size = 50  # Size of each Sudoku cell in pixels
minor_grid_size = 1  # Thickness of the minor grid lines (between cells)
major_grid_size = 3  # Thickness of the major grid lines (between 3x3 boxes)
buffer = 15  # Padding around the game board

button_height = 50  # Height of buttons
button_width = 125  # Width of buttons
button_border = 2  # Border thickness of buttons
...
class RectCell(pygame.Rect):
    ...
    '''
    def __init__(self, left, top, row, col): # x, y (position) row idx, col idx 
...
def create_cells():
    ...
    cells = [[] for _ in range(9)] # generate 9 lists []
...
def read_puzzle(address):
    f = open(address, 'r')
    sudoku_grid = [list(map(int, line.split())) for line in f]
    return sudoku_grid


def play():
    '''Contains all the functionality for playing a game of Sudoku.'''
    initial = read_puzzle("./sample_sudoku/Test1.txt")
    game = Sudoku(initial)
    print(game.__str__())
    cells = create_cells()
    active_cell = None
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                # Test if active cell is empty == not the default value -> set it to non active
