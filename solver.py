...
class Sudoku:
    ...
    def __init__(self, board):
        ...
        self.board = [] # contain all Cell instances
        for row in range(9):
            self.board.append([])
            for col in range(9):
                if board[row][col] == 0:
    ...
    def check_move(self, cell, num):
        ...
        for row in range(cell.row // 3 * 3, cell.row // 3 * 3 + 3): # [0 3) [3 6) [6 9)
    ...
    def get_empty_cell(self):
        '''Returns an empty cell to start the solving algorithm. Returns False if all cells are filled in.'''
        for row in range(9):
            for col in range(9):
    ...
    def solve(self):
        ...
        '''
        cell = self.get_empty_cell() # starting cell
